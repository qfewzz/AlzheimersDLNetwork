...
class Network(nn.Module):
    ...
    def __init__(self, input_channels, input_shape, output_size, lstm_layers=1):
        ...
        def dimensions_after_convolution(kernel, stride, padding, input_shape):
            ...
            output_depth = math.floor(
                (input_shape[0] + 2 * padding - kernel + 1) / stride
                + (stride - 1) / stride
            )
            output_height = math.floor(
                (input_shape[1] + 2 * padding - kernel + 1) / stride
                + (stride - 1) / stride
            )  # the total height is the input plus twice the padding
            output_width = math.floor(
                (input_shape[2] + 2 * padding - kernel + 1) / stride
                + (stride - 1) / stride
            )
        ...
        current_shape = dimensions_after_convolution(
            kernel_size, 1, padding, input_shape
        )
        self.pool1 = nn.MaxPool3d(kernel_size)
        current_shape = dimensions_after_convolution(
            kernel_size, kernel_size, padding, current_shape
        )
        self.convolution2 = nn.Conv3d(10, 5, kernel_size, padding=padding)
        current_shape = dimensions_after_convolution(
            kernel_size, 1, padding, current_shape
        )
        ...
        print(
            f"For the specified shape, the LSTM input dimension has been calculated at {lstm_input_dimensions}."
        )
        if lstm_input_dimensions > 1000:
            print(
                "This seems very large. Perhaps you should add some more layers to your network, or increase the kernel size."
            )
    ...
    def forward(self, MRI):
        feature_space = self.convolution3(
